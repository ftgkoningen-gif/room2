...
create table if not exists wk2026_wedstrijden (
  ...
  pens_thuis     int,
  pens_uit       int,
  status         text,                 -- 'gepland' | 'gespeeld' | 'verwerkt'
...
create table if not exists wk2026_events (
  ...
  type            text not null,       -- gespeeld45 | velddoelpunt | assist | strafschop | geleKaart | directeRood | eigenGoal | cleanSheet45 | tegendoelpunt
  detail          text
);
